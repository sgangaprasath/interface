...
export default function ProgressSlider({ items }: { items: Item[] }) {
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto text-center">
          ...
          {items.map((item, index) => (
            ...
            >
              <Image
                ...
                alt={item.desc}
              />
              <span className="block py-2 opacity-50 text-sm font-medium text-slate-900 mb-2">
                {item.desc}
              </span>
            </Transition>
          ))}
      ...
      {/* Buttons */}
      <div className="max-w-xs sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-2 md:grid-cols-2 gap-4">
        {items.map((item, index) => (
          <button
            key={index}
            className="rounded focus:outline-none focus-visible:ring focus-visible:ring-green-300 group"
            onClick={() => {
              setActive(index);
              setProgress(0);
            }}
          >
            ...
            >
              ...
              <span 
                className="block relative w-10 h-1 rounded-full"
                role="progressbar"
                aria-valuenow={active === index ? progress : 0}
              >
                <span className="block relative w-full bg-slate-200 h-1 rounded-full" role="progressbar" aria-valuenow={active === index ? progress : 0}></span>
                <span
